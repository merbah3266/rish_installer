...
if [ "$OFFLINE_OK" -eq 0 ]; then
  case "$SOURCE_MODE" in
    ...
    default|thedjchi|custom_repo)
      ...
      step "Fetching from $REPO..."; URL="$(curl -fsSL "https://api.github.com/repos/$REPO/releases/latest" | sed -n 's/.*"browser_download_url":[[:space:]]*"\([^"]*\.apk\)".*/\1/p')"; [ -z "$URL" ] && cancel "URL fetch failed. Cancelled."
