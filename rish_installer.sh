...
if [ "$PLAN_A" -eq 1 ]; then
  ...
  curl -fsSL "https://raw.githubusercontent.com/merbah3266/rish_installer/main/busybox/$ARCH/busybox" -o "$BB" || err "BB download failed"
...
if [ "$OFFLINE_OK" -eq 0 ]; then
  case "$SOURCE_MODE" in
    ...
    default|thedjchi|custom_repo)
      ...
      URL="$(curl -fsSL "https://api.github.com/repos/$REPO/releases/latest" | sed -n 's/.*"browser_download_url":[[:space:]]*"\([^"]*\.apk\)".*/\1/p')"
